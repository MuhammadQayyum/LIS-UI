...
const AllPhysicians = (props) => {
  ...
  const deletePhysicianHandler = ((id) => {
    if(window.confirm("Do you want to remove?")){
      fetch(api.delete("/deletePhysician/"+id).then(() =>
    ...
    const phy = props.physicians.map((physician) => {
        return(
            ...
            <tr>
            <td class = "r" ><Link to={'/editPhysician/' + physician.phyId}>{physician.phyId}</Link></td>
            ...
            <td>
              <a className='fa fa-trash-o' style={{color:"red", marginTop: "7px"}} onClick={() => {deletePhysicianHandler(physician.phyId)}}></a>
            </td>
          </tr>
    ...
    <li className="nav-item">
    <Link to={'/addPhysician'} ><a className="nav-link">New Physician</a></Link>
    </li>
    </ul>
